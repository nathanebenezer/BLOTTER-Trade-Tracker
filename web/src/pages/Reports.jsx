...
import { selectedClosed } from "../lib/filter.js";
...
export default function Reports({ filter }) {
  const { trades, meta } = useStore();
  const [eqMode, setEqMode] = useState("dollar");

  const closed = selectedClosed(trades, filter);
  const s = computeStats(closed);
  const series = equitySeries(closed, { mode: eqMode, baseline: meta?.equity_baseline || 0 });

  if (s.n === 0) {
    return (
      <div className="pagestub">
        <b>No closed trades in this period</b>
        Adjust the filter above, or close out a trade on the Trades page.
      </div>
    );
  }
  ...
  return (
    <>
      <div className="stats">
        <Card k="Net realised" v={fMoney(s.net, true)} vc={cls(s.net)} sub={`${s.n} trade${s.n !== 1 ? "s" : ""}`} />
